...
class EPAModel(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        return torch.sum(self.output(hidden).permute(1, 0, 2).squeeze() * (1 - attn_mask), dim=1) / torch.sum(1 - attn_mask, dim=1)
...
if __name__ == "__main__":
    ...
    model = EPAModel(69, 3, 256, 2).float()
        ...
        for item_x, item_y in train_loader:
            ...
            if step % 100 == 0:
                ...
                for val_item_x, val_item_y in val_loader:
                    ...
                    val_truth = val_items_y.to(device)[:,0]
